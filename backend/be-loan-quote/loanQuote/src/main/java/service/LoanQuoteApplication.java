...
import exception.LoanQuoteParameterValidationException;
...
public final class LoanQuoteApplication {
     ...
     */
    public static LoanQuoteDTO calculation(LoanQuote loanQuote) {
        LoanQuoteDTO loanQuoteDTO = new LoanQuoteDTO(0);
        final LoanQuoteCalculator loanQuoteCalculator;
        final int loanAmount;
        try {
            loanQuoteCalculator = createLoanQuoteCalculator("dom");
            loanAmount = getLoanAmount(loanQuote.getLoanAmount());
            loanQuoteDTO.setLoanAmount(loanAmount);
    ...
    public static void calculation(final String[] args) {
...
//        }
    ...
    static LoanQuoteCalculator createLoanQuoteCalculator(final String marketFilePath) throws LoanQuoteParameterValidationException {
        ...
        final List<Lender> lenders = new ArrayList<Lender>();
...
//                    .parse();
     ...
     */
    static int getLoanAmount(final int loanAmountAsString) throws LoanQuoteParameterValidationException {
...
//        }
        ...
        if (loanAmountAsString < MIN_LOAN_AMOUNT || MAX_LOAN_AMOUNT < loanAmountAsString || loanAmountAsString % LOAN_AMOUNT_INCREMENT != 0) {
            throw new LoanQuoteParameterValidationException("Invalid loan amount, must be any 100 increment between 1000-15000 inclusive: " + loanAmountAsString);
        }

        // parseInt allows leading zero or leading plus, we shouldn't
//        final char loanAmountLeadingChar = loanAmountAsString.charAt(0);

        if (loanAmountAsString == '0') {
